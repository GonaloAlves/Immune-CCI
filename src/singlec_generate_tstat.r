...
# Define dirs
checkpoint_dir = sprintf("%s/0_checkpoint", output_dir)
gsea_dir = sprintf("%s/%s_%s/14_gsea", output_dir, cell_suffix, f_suffix)

# Datasets
datasets_divided = c('Immune', 'Neuron', 'Meningeal_Vascular', 'Astrocyte', 'Oligodendrocyte')
neigh = 15  # neighbour
resolution = list()
resolution[['Immune']] = c(0.7)
resolution[['Meningeal_Vascular']] = c(1.4)
resolution[['Astrocyte']] = c(0.7)
resolution[['Oligodendrocyte']] = c(0.9)
resolution[['Neuron']] = c(5)
...
for (d in datasets_divided)
{
  ...
  dest = sprintf("%s/adata_final_%s_raw_norm_ranked.h5ad", checkpoint_dir, d)
  print(dest)
  ...
  ### Cluster analysis ###
  clusters = sprintf("leiden_n%i_r%s", neigh, resolution[[d]])
  ...
  for (c in levels(ann$obs[[clusters]]))
  {
    ...
    # Write to file #
    printf("Write table...\n")
    dest = sprintf("%s/%s_%s_c%s_tstat.txt", gsea_dir, d, clusters, c)
