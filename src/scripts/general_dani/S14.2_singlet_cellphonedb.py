...
def build_meta_file(adata: sc.AnnData,
    ...
    if group_by is None:    # All cells
        ...
        meta_file_path = f"{cellphonedb_dir}/adata_final_merged_metadata.tsv"
        ...
        meta_file_path = f"{cellphonedb_dir}/adata_final_merged_{subset}_metadata.tsv"
        print(meta_file_path)
        meta_df.to_csv(meta_file_path, sep='\t', index=False)
        return meta_file_path
    else:
...
def build_deg_file(file_path: str,
    ...
    print("Save deg file...")
    deg_file_path = f"{cellphonedb_dir}/{out_file}"
    print(deg_file_path)
...
def start() -> None:
    ...
    from src.globals import checkpoint_dir, cpdb_dir, cellphonedb_dir, marker_genes_dir
    ...
    if deg_analysis:
        ...
        deconvoluted, means, relevant_interactions, significant_means = cpdb_degs_analysis_method.call(
            ...
            separator='|',                                            # Sets the string to employ to separate cells in the results dataframes "cellA|CellB".
            debug=False,                                              # Saves all intermediate tables emplyed during the analysis in pkl format.
            output_path=cellphonedb_dir,                              # Path to save results
            output_suffix="final_merged"                              # Replaces the timestamp in the output files by a user defined string in the  (default: None)
        )

        # For Sham
        degfile_sham = build_deg_file(f"{marker_genes_dir}/adata_final_merged_marker_genes_leiden_merge_sham.15.txt",
                                      "adata_final_merged_sham.15_deg.tsv")
        deconvoluted, means, relevant_interactions, significant_means = cpdb_degs_analysis_method.call(
            cpdb_file_path=cpdb_file_path,                            # mandatory: CellPhoneDB database zip file.
            meta_file_path=metafile_sham,                             # mandatory: tsv file defining barcodes to cell label.
            counts_file_path=counts_file_path,                        # mandatory: normalized count matrix.
            degs_file_path=degfile_sham,                              # mandatory: tsv file with DEG to account.
            counts_data='hgnc_symbol',                                # defines the gene annotation in counts matrix.
            threshold=0.3,                                            # defines the min % of cells expressing a gene for this to be employed in the analysis.
            result_precision=4,                                       # Sets the rounding for the mean values in significan_means.
            separator='|',                                            # Sets the string to employ to separate cells in the results dataframes "cellA|CellB".
        ...
        # For Injured_15
        degfile_injury_15 = build_deg_file(f"{marker_genes_dir}/adata_final_merged_marker_genes_leiden_merge_injured.15.txt",
                                           "adata_final_merged_injured.15_deg.tsv")
        deconvoluted, means, relevant_interactions, significant_means = cpdb_degs_analysis_method.call(
            cpdb_file_path=cpdb_file_path,                            # mandatory: CellPhoneDB database zip file.
            meta_file_path=metafile_injury_15,                        # mandatory: tsv file defining barcodes to cell label.
            counts_file_path=counts_file_path,                        # mandatory: normalized count matrix.
            degs_file_path=degfile_injury_15,                         # mandatory: tsv file with DEG to account.
            counts_data='hgnc_symbol',                                # defines the gene annotation in counts matrix.
            threshold=0.3,                                            # defines the min % of cells expressing a gene for this to be employed in the analysis.
            result_precision=4,                                       # Sets the rounding for the mean values in significan_means.
            separator='|',                                            # Sets the string to employ to separate cells in the results dataframes "cellA|CellB".
        ...
        # For Injured_60
        degfile_injury_60 = build_deg_file(f"{marker_genes_dir}/adata_final_merged_marker_genes_leiden_merge_injured.60.txt",
                                           "adata_final_merged_injured.60_deg.tsv")
        deconvoluted, means, relevant_interactions, significant_means = cpdb_degs_analysis_method.call(
            cpdb_file_path=cpdb_file_path,                            # mandatory: CellPhoneDB database zip file.
            meta_file_path=metafile_injury_60,                        # mandatory: tsv file defining barcodes to cell label.
            counts_file_path=counts_file_path,                        # mandatory: normalized count matrix.
            degs_file_path=degfile_injury_60,                         # mandatory: tsv file with DEG to account.
            counts_data='hgnc_symbol',                                # defines the gene annotation in counts matrix.
            threshold=0.3,                                            # defines the min % of cells expressing a gene for this to be employed in the analysis.
            result_precision=4,                                       # Sets the rounding for the mean values in significan_means.
            separator='|',                                            # Sets the string to employ to separate cells in the results dataframes "cellA|CellB".
